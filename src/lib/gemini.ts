...
`;
  ...
  const result = await callGeminiRaw(prompt, "FarmerInsights");

  if (result.isAi && result.text) {
    const parsed = extractJsonFromText(result.text);
    if (
      parsed &&
      typeof parsed.demandInsight === "string" &&
      typeof parsed.inventoryRecommendation === "string" &&
      typeof parsed.sellingRecommendation === "string" &&
      typeof parsed.shortExplanation === "string"
    ) {
      return {
        demandInsight: parsed.demandInsight.trim(),
        inventoryRecommendation: parsed.inventoryRecommendation.trim(),
        sellingRecommendation: parsed.sellingRecommendation.trim(),
        shortExplanation: parsed.shortExplanation.trim(),
        isAiGenerated: true,
...
`;
  ...
  const result = await callGeminiRaw(prompt, "ForecastExplanation");

  if (result.isAi && result.text) {
    const parsed = extractJsonFromText(result.text);
    if (parsed && typeof parsed.explanation === "string") {
      return {
        explanation: (parsed.explanation as string).slice(0, 300),
        factors: {
...
`;
  ...
  const result = await callGeminiRaw(prompt, "FarmerAssistant");
...
`;
  ...
  const result = await callGeminiRaw(prompt, "DiscussionSummary");
...
`;
  ...
  const result = await callGeminiRaw(prompt, "CommunityAssistant");

  if (result.isAi && result.text) {
    const parsed = extractJsonFromText(result.text);
    if (parsed && typeof parsed.reply === "string" && Array.isArray(parsed.suggestedActions)) {
      return {
...
`;
  ...
  const result = await callGeminiRaw(prompt, "PriceRecommendationAdvisor");
